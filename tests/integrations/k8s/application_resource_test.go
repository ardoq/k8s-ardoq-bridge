...
var _ = Describe("ApplicationResource", func() {
	var kubectlClient = helper.KubectlClient()
	var deployResourceName = "int-test-" + helper.RandomString(10)
	var stsResourceName = "int-test-" + helper.RandomString(10)
	...
	Context("Deployment tests", Ordered, func() {
		...
		It("Can not find deleted Deployments", func() {
			...
			resourceClient := helper.KubectlClient().AppsV1().Deployments(v1.NamespaceDefault)
	...
	Context("StatefulSet tests", Ordered, func() {
		BeforeAll(func() {
			...
			stsClient := helper.KubectlClient().AppsV1().StatefulSets(v1.NamespaceDefault)
		...
		It("Can not find deleted StatefulSets", func() {
			...
			deletePolicy := metav1.DeletePropagationForeground
			resourceClient := helper.KubectlClient().AppsV1().StatefulSets(v1.NamespaceDefault)
